...
export const addProduct = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        res.status(201).json({ message: "Product added successfully", data: {name, description, price, createdBy} });
    } catch (error) {
...
export const editProduct = async (req: Request, res: Response) => {
    try {
        ...
        const success = await productModel.updateProduct(id, name, description, price);
        if (success) {
